...
if len(sys.argv)>2:
  ...
  print('  usage: plot_bott_baltic.py file.nc variable')
  sys.exit(1)
...
if os.path.isfile('proj_bottbaltic.pickle'):
    (proj,)=np.load('proj_bottbaltic.pickle')
else:
    ...
    f=open('proj_bottbaltic.pickle','wb')
    pickle.dump((proj,),f)
    f.close()
...
for tidx,t in enumerate(dates):
  ...
  if plot_surface:
    ...
    savefig('jpgs/%s/%s_baltic_surface_%s.jpg'%(varname,varname,tstring),dpi=200)
    close()
  ...
  if plot_bottom:
    ...
    cb.ax.set_title(u'%s\n'%(cbtitle),size=10.)
    tstring = t.strftime('%Y%m%d-%H%M')
    savefig('jpgs/%s/%s_baltic_bottom_%s.jpg'%(varname,varname,tstring),dpi=200)
    #show()
    close()
