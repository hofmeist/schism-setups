...
depthv=[]
for n,(x,y) in enumerate(zip(tri.x,tri.y)):
  d = interp(x,[0.0,0.3*length,0.7*length,length],[50.0,50.0,10.0,10.0])
  depthv.append(d)

f.write('hgrid.gr3\n')
f.write('%d %d\n'%(len(tri.triangles),len(tri.x)))

for n,(x,y,depth) in enumerate(zip(tri.x,tri.y,depthv)):
...
nleft = len(where(tri.x==length)[0])
nright = len(where(tri.x==0.0)[0])
# number of land boundaries
f.write('2\n')
# number of land boundary nodes
f.write('%d\n'%(nupper+nlower+nright+nleft-2))
# number of lower land boundary nodes and nodes
f.write('%d 0\n'%(nlower+nright-1))
print(nupper)
for i in where(tri.y==-dx)[0][:-1]:
  f.write('%d\n'%(i+1))
...
saltv=[]
for n,(x,y) in enumerate(zip(tri.x,tri.y)):
  salt = interp(x,[0.0,0.45*length,0.55*length,length],[36.0,36.0,35.0,35.0])
  saltv.append(salt)
...
write_gr3(tri,'diffmin.gr3',const=0.001)
write_gr3(tri,'rough.gr3',const=0.001)
write_gr3(tri,'xlsc.gr3',const=0.1)
