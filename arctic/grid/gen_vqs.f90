...
!     Read  vgrid.in
      ...
      hsm=(/10,13,16,19,22,25,28,31,34,37,40,43,46,49,52,55,60,65,70,75,80,85,90,95,100,110,120,130,140,150,175,200,225,250,300,400,500,750,1000,1250,1500,1750,2000,2250,2500,2750,3000,3250,3500,3750,4000,4500,5000/)
      ...
      nv_vqs(1:m_vqs)=(/(6+1*(i-1),i=1,m_vqs)/) !# of levels for each master grid (increasing with depth)
...
!     Generate a master vgrid (z_mas)
      ...
      do m=1,m_vqs
        do k=1,nv_vqs(m)
          ...
          ! for z-level-like coordinates
          sigma=(k-1.0)/(1.0-nv_vqs(m)+m)
...
!          Option 2: S
     ...
     &theta_b*(tanh(theta_f*(sigma+0.5))-tanh(theta_f*0.5))/2/tanh(theta_f*0.5)
          ...
          ! for z-level-like coordinates
          if (k<nv_vqs(m)-m) then
            z_mas(k,m) = hsm(1)*sigma
          else
            z_mas(k,m) = -hsm(k-nv_vqs(m)+m+1)
          end if
        enddo !k
