...
if os.path.isfile('plotearthproj.pickle'):
    with open('plotearthproj.pickle','rb') as f:
      (proj,)=pickle.load(f)
else:
    if True:
      proj = Basemap(projection='ortho',lat_0=30,lon_0=30.0,resolution='l')
    ...
    f=open('plotearthproj.pickle','wb')
    pickle.dump((proj,),f)
    f.close()
...
if True:
  if True:
    ...
    cbtitle = titlestr
    # draw coastlines, country boundaries, fill continents.
    proj.drawcoastlines(linewidth=0.25)
    proj.drawcountries(linewidth=0.25)
    proj.fillcontinents(color=(0.9,0.9,0.8),lake_color='aqua')
    # draw the edge of the map projection region (the projection limb)
    proj.drawmapboundary(fill_color='aqua')
    # draw lat/lon grid lines every 30 degrees.
    proj.drawmeridians(np.arange(0,360,30),linewidth=0.1)
    proj.drawparallels(np.arange(-90,90,30),linewidth=0.1)
    if False:
    ...
    savefig('jpgs/vel_earth/meantransport_%s.jpg'%(tstring),dpi=600)
    show()
    close()
