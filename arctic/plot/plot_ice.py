...
var = ncv[varname]
if varname!='ICE_tracer_1':
  maskvar = ncv['ICE_tracer_1']
else:
  maskvar=var
...
for tidx,t in enumerate(dates):
  ...
  vmask = maskvar[tidx+tidx_offset].squeeze()
  emask = vmask[nv].mean(axis=1)
  plot_surface=True
  plot_bottom=False
  if plot_surface:
    fig=figure()
    ...
    if varname=='elev':
      cmap=cm.RdYlGn
      cmap.set_under('gray')
    tripcolor(x,y,nv,vs,mask=emask<0.02,cmap=cmap,rasterized=True)
    if varname=='salt':
      clim(5,35)
    ...
    savefig('jpgs/%s/ice_%s_%s.jpg'%(varname,varname,tstring),dpi=200)
    close()
