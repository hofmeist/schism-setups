...
def create_workdir(path,origpath=os.path.join(os.environ['HOME'],'schism/setups/arctic')):
  """
  ...
  copylist=['param.default','ice.nml','gotmturb.nml','fabm.nml','albedo.gr3','rough.gr3','bctides.in']
  for f in copylist:
    try:
...
runid='arctic'+id
if debug: out="1 1"
workdir='/scratch/g/'+os.environ['USER']+'/schism-results/%s'%runid
